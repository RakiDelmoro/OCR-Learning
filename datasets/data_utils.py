...
def real_data_processing(image_numpy_array):
    average_pixel = np.average(image_numpy_array) - 25
    background_condition = image_numpy_array > average_pixel
    # text_condition = image_numpy_array < average_pixel

    image_numpy_array[background_condition] = 255
    # image_numpy_array[text_condition] = 0
    
    # light_noise = np.random.randint(128, 256, size=image_numpy_array.shape).astype(np.uint8)
    # dark_noise = np.random.randint(0, 127, size=image_numpy_array.shape).astype(np.uint8)
...
def inference_data_processing(image_arr):
    average_pixel = np.average(image_arr) - 10
    background_condition = image_arr > average_pixel
    text_condition = image_arr < average_pixel
    image_arr[background_condition] = 255
    image_arr[text_condition] = 0

    return cv.bitwise_not(image_arr)
...
def image_processing_for_real_data(image_as_array):
    ...
    image = real_data_processing(rotate_img)
