...
class CnnFeatureExtractor(nn.Module):
    def __init__(self, config=EncoderConfig):
        ...
        self.position_embeddings = nn.Parameter(torch.zeros(1, 159, config.embedding_dimension))
...
class Encoder(nn.Module):
    ...
    def forward(self, image: torch.Tensor, target=None):
        ...
        if training_mode:
            ...
            for each in range(batch):
                target_length[each] = actual_tokens_length[each]
...
# model = Encoder()
# input_image = torch.randn(16, 1, 64, 768)
# target = torch.randint(low=1, high=EncoderConfig.num_classes, size=(16, 10))
